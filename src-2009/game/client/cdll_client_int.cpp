...
int CHLClient::Init( CreateInterfaceFn appSystemFactory, CreateInterfaceFn physicsFactory, CGlobalVarsBase *pGlobals )
{
	...
	// Initialize Maxsi Distribution.
	if ( d_MaxsiDistribution.InitializeMaxsiDistribution() == false )
	{
		return false; // If Maxsi Distribution tells us to return false, do so.
	}
